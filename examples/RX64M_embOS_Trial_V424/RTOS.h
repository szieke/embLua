...
/*********************************************************************
*
*       Chip specifics RENESAS RX and CCRX compiler
*
**********************************************************************
*/

#include <string.h>               // memset and others...
#include <machine.h>              // intrinsics
...
#define OS_BIG_ENDIAN             __BIG                      // 0 => little endian
...
#define OS_SADDR                                             // OS-data may reside anywhere in memory
#define OS_STACKPTR                                          // Always treat as huge (because SP is 32 bit)
...
#define OS_USEPARA(p)             ((void)(p))                // Avoid compiler warning

/*****  Core specific configuration *********************************/
#define OS_INTERRUPTS_ARE_NESTABLE_ON_ENTRY   (0)
#define OS_SWITCH_FROM_INT_MODIFIES_STACK     (1)

#define OS_IPL_DI_DEFAULT (4uL << 24)   // Contains IPL bits only, at correct position
#define OS_IPL_EI_DEFAULT (0uL << 24)   // Contains IPL bits only, at correct position

#define OS_IPL_DI_TYPE    unsigned long
#define OS_IPL_EI_TYPE    unsigned long
...
*
*  RXv1 core has only one 64bit ACC register
*  RXv2 core has two 72bit ACC register
*/
typedef struct {
  OS_U32 Counters;
  OS_U32 OS_REG_R6;
...
typedef struct {
#if (defined (__RXV1)) && (__RXV1 == 1)
  OS_U32 OS_REG_ACCHI;
  OS_U32 OS_REG_ACCLO;
#else
  OS_U32 OS_REG_ACC0HI;
  OS_U32 OS_REG_ACC0MI;
  OS_U32 OS_REG_ACC0LO;
  OS_U32 OS_REG_ACC1HI;
  OS_U32 OS_REG_ACC1MI;
  OS_U32 OS_REG_ACC1LO;
#endif
  OS_U32 OS_REG_R1;
  OS_U32 OS_REG_R2;
  OS_U32 OS_REG_R3;
...
/*********************************************************************
*
*       Port specific (non OS generic) functions and data
*/
extern  void OS_EnableInterrupt(void);
extern  void OS_DisableInterrupt(void);

#define OS_DI() OS_DisableInterrupt()
#define OS_EI() OS_EnableInterrupt()

#define OS_ENABLE_INTS_SAVE_IPL()     \
  { unsigned int  Flg;                \
                                      \
  set_psw(get_psw() | (1uL << 16));   \
  Flg = OS_Ipl_EI;                    \
  OS__SaveIPL(); {
...
/*********************************************************************
*
*       Global interrupt disable/enable support, generic functions
*       since embOS version 3.90
*       Implemented here, using intrinsics
*/
#define OS_INTERRUPT_MaskGlobal()      clrpsw_i();
#define OS_INTERRUPT_UnmaskGlobal()    setpsw_i();

#define OS_INTERRUPT_PreserveGlobal(p) (*(p) = (OS_U32) get_psw() & 0x010000)

#define OS_INTERRUPT_RestoreGlobal(p)  set_psw((OS_U32) get_psw() | *(p))

#define OS_INTERRUPT_PreserveAndMaskGlobal(p)  { \
  (*(p) = (OS_U32) get_psw() & 0x010000);        \
  clrpsw_i();                                    \
}
...
*/
#define OS_INT_PRIO_PRESERVE(p)  (*(p) = (OS_U32) get_psw() & 0x0F000000)
#define OS_INT_PRIO_RESTORE(p)   set_psw((get_psw() & 0xF0FFFFFF) | *(p))
...
#ifdef  __BIG
  #define OS_ChangeTask OS_ChangeTask_BE
#else
  #define OS_ChangeTask OS_ChangeTask_LE
#endif
